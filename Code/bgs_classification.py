...
while(1):
    ret, frame2 = cap.read()
    rgb = fgbg.apply(frame2, learningRate = 10.0/history)
    # print (rgb[0][0])
    ...
    rgb = cv2.morphologyEx(rgb, cv2.MORPH_OPEN, kernelo)
    thresh = 4
    rgb = cv2.threshold(rgb, thresh, 255, cv2.THRESH_BINARY)[1]
    # rgb2 = cv2.morphologyEx(rgb2, cv2.MORPH_OPEN, kernelo)
    if count%5==0:
        kerneld = np.ones((20,20),np.uint8)
        rgb = cv2.dilate(rgb, kerneld, iterations = 4)
        (_,cnts, _) = cv2.findContours(rgb.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        for c in cnts:
        ...
        rgb = cv2.resize(rgb, (int(width/red),int(height/red)))
        rgb3 = cv2.cvtColor(rgb,cv2.COLOR_GRAY2BGR)
        w1 = w2 = int(width/red)
        ...
        # cv2.waitKey(1)
        # cv2.imwrite('result3/bgs/'+filename[15:-5]+str(count)+'.jpg',newimg)
